...
func TestUnsafeOr(t *testing.T) {
	...
	b11 := NewBitmap()
	b22 := NewBitmap()
	_, err = b11.FromUnsafeBytes(data1)
	require.NoError(t, err)
	_, err = b22.FromUnsafeBytes(data2)
	require.NoError(t, err)

	for i := range b11.highlowcontainer.containers {
		require.True(t, b11.highlowcontainer.needsCopyOnWrite(i), i)
	}
	b22Containers := map[string]bool{}
	for i := range b22.highlowcontainer.containers {
		require.True(t, b22.highlowcontainer.needsCopyOnWrite(i), i)
		b22Containers[fmt.Sprintf("%p", b22.highlowcontainer.containers[i])] = true
	}

	require.True(t, b11.Equals(b1))
	require.True(t, b22.Equals(b22))
	b11.UnsafeOr(b22)
	require.Equal(t, uint64(0), b22.GetCardinality())
	for i := range b11.highlowcontainer.containers {
		if b22Containers[fmt.Sprintf("%p", b11.highlowcontainer.containers[i])] {
			require.True(t, b11.highlowcontainer.needsCopyOnWrite(i), i)
		}
	}
